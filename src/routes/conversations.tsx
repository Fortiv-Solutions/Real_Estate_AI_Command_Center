...
const messages = [
  {
    from: "ai",
    t: "10:24",
    text: "Hello Priya! Welcome to Fortiv 🏠 I'd love to help you find your perfect home. May I ask your budget?",
  },
  { from: "user", t: "10:25", text: "Around 60-75 lakhs, 3BHK in Vesu" },
  {
    from: "ai",
    t: "10:25",
    text: "Excellent choice. We have 11 available 3BHK units at Skyline Heights, Vesu, starting ₹68L. Possession is ready. Would you like to see the floor plan and price breakdown?",
  },
  { from: "user", t: "10:31", text: "Yes please, and can I visit this Saturday?" },
  {
    from: "ai",
    t: "10:31",
    text: "Sharing brochure 📎 Skyline_3BHK_Brochure.pdf. For Saturday I can book a visit at 11:00 am with Nilesh — does that work?",
  },
...
function Conversations() {
  return (
    <AppShell
      title="Conversations"
      subtitle="WhatsApp · Calls · Email · Web chat — handled by AI, escalated to humans"
    >
      <Card className="overflow-hidden grid grid-cols-12 h-[calc(100vh-220px)] min-h-[560px]">
        ...
        <div className="col-span-12 md:col-span-4 lg:col-span-3 border-r border-border/60 flex flex-col">
          <div className="p-3 border-b border-border/60">
            <div className="relative">
              ...
              <input
                className="w-full h-9 pl-8 pr-3 rounded-lg bg-muted text-[13px] focus:outline-none"
            ...
            <div className="flex gap-1 mt-2">
              {["All", "Unread", "AI handled", "Escalated"].map((t, i) => (
                <button
                  key={t}
                  className={`text-[11px] px-2 py-1 rounded-md ${i === 0 ? "bg-ink text-cream" : "text-muted-foreground hover:bg-muted"}`}
                >
                  {t}
                </button>
              ))}
          ...
          <ul className="flex-1 overflow-y-auto">
            {threads.map((th, i) => (
              <li
                key={th.n}
                className={`px-3 py-3 border-b border-border/40 cursor-pointer flex gap-3 ${i === 0 ? "bg-muted/60" : "hover:bg-muted/40"}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5">
                    ...
                    {th.unread > 0 && (
                      <span className="text-[10px] bg-ink text-cream rounded-full px-1.5 min-w-[18px] h-[18px] grid place-items-center">
                        {th.unread}
                      </span>
                    )}
                  </div>
        ...
        <div className="col-span-12 md:col-span-5 lg:col-span-6 flex flex-col">
          <div className="p-4 border-b border-border/60 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-accent to-amber-200 grid place-items-center text-[12px] font-semibold text-ink">
                PS
              </div>
              <div>
                <div className="text-[13px] font-medium flex items-center gap-2">
                  Priya Shah <Pill tone="hot">Hot · 92</Pill>
                </div>
                <div className="text-[11px] text-muted-foreground">
                  WhatsApp · +91 98250 22118 · last seen 2m ago
                </div>
              </div>
            </div>
            <div className="flex gap-1">
              <button className="h-9 w-9 grid place-items-center rounded-lg hover:bg-muted">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-3">
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[84%] sm:max-w-[70%] rounded-lg px-4 py-2.5 text-sm ${
                    m.from === "user"
                ...
                >
                  ...
                  <div>{m.text}</div>
                  <div
                    className={`text-[10px] mt-1 ${m.from === "user" ? "text-cream/60" : "text-muted-foreground"}`}
                  >
                    {m.t}
                  </div>
                </div>
          ...
          <div className="p-3 border-t border-border/60">
            <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2">
              ...
              <input
                ...
                placeholder="Type or let AI respond…"
              />
              <button className="text-[11px] text-muted-foreground hover:text-foreground flex items-center gap-1">
                <Sparkles className="h-3 w-3" />
                Suggest
              </button>
              <button className="h-8 w-8 grid place-items-center rounded-full bg-ink text-cream">
        ...
        <div className="hidden lg:flex col-span-3 border-l border-border/60 flex-col p-5 overflow-y-auto">
          ...
          <div className="mt-5 p-3 rounded-lg bg-accent/30 border border-accent/40">
            <div className="text-[11px] font-medium flex items-center gap-1">
              <Sparkles className="h-3 w-3" />
              AI suggestion
            </div>
            <p className="text-[12px] mt-1.5">
              Recommend Skyline Heights 3BHK. Buyer matches 4/4 qualification criteria. Visit on Sat
              11am — assign Nilesh.
            </p>
          </div>
        </div>
