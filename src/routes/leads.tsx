...
function Leads() {
  return (
    <AppShell
      ...
      actions={
        <>
          <button className="h-10 px-3 rounded-lg border border-border bg-card text-[13px] flex items-center gap-2"><Filter className="h-3.5 w-3.5" />Filters</button>
          <button className="h-10 px-3 rounded-lg border border-border bg-card text-[13px] flex items-center gap-2"><Download className="h-3.5 w-3.5" />Export</button>
    ...
    >
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        {[
          ...
          <button key={t.l} className={`shrink-0 h-9 px-3.5 rounded-full text-[12px] flex items-center gap-2 border ${t.active ? "bg-ink text-cream border-ink" : "border-border bg-card hover:bg-muted"}`}>
            {t.l}<span className={`text-[10px] px-1.5 py-0.5 rounded ${t.active ? "bg-cream/15" : "bg-muted"}`}>{t.c}</span>
          </button>
        ))}
      </div>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-[0.12em] text-muted-foreground border-b border-border/60">
              {["Lead","Source","Interest","Budget","Score","Status","Agent",""].map((h) => (
          ...
          <tbody>
            {leads.map((l, i) => (
              <tr key={i} className="border-b border-border/40 last:border-0 hover:bg-muted/40">
                ...
                <td className="px-4 py-3"><Pill>{l.src}</Pill></td>
                <td className="px-4 py-3">
                  <div>{l.cfg} · {l.area}</div>
                </td>
                <td className="px-4 py-3 font-medium">{l.budget}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className="font-display text-lg leading-none">{l.score}</div>
                    <Pill tone={l.tone as any}>{l.tone === "hot" ? <><Flame className="h-2.5 w-2.5"/>Hot</> : l.tone === "warm" ? "Warm" : "Cold"}</Pill>
                  </div>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-1">
                    <button className="h-8 w-8 grid place-items-center rounded-md hover:bg-muted"><Phone className="h-3.5 w-3.5"/></button>
                    <button className="h-8 w-8 grid place-items-center rounded-md hover:bg-muted"><MessageCircle className="h-3.5 w-3.5"/></button>
                    <button className="h-8 px-2 rounded-md hover:bg-muted text-[11px] flex items-center gap-1">Open <ChevronDown className="h-3 w-3"/></button>
                  </div>
                </td>
              </tr>
            ))}
