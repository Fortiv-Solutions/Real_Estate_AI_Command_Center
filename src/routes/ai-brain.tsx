...
function AIBrain() {
  return (
    <AppShell
      title="AI Brain · Data Layer & Security"
      subtitle="Ask anything about your business · M12 unified intelligence layer"
    >
      <Card className="p-6 bg-ink text-cream relative overflow-hidden">
        <div
          className="absolute -right-32 -top-32 h-96 w-96 rounded-full"
          style={{
            background: "radial-gradient(circle, oklch(0.88 0.16 92 / 0.45), transparent 60%)",
          }}
        />
        <div className="relative">
          ...
          <h2 className="font-display text-3xl sm:text-4xl mt-2 max-w-3xl leading-tight">
            Ask me about leads, projects, revenue, agents — in English, Hindi or Gujarati.
          </h2>
          <div className="mt-6 flex gap-2 items-center bg-cream/10 rounded-lg px-4 py-3 border border-cream/15 max-w-3xl">
            <Sparkles className="h-4 w-4 opacity-70" />
            <input
              className="flex-1 bg-transparent text-[14px] placeholder:text-cream/50 focus:outline-none"
              placeholder="e.g. Which 3BHK projects had highest engagement this week?"
            />
            <button
              aria-label="Send prompt"
              className="h-9 w-9 grid place-items-center rounded-md bg-accent text-ink"
            >
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {[
              ...
              <button
                key={s}
                className="text-[12px] px-3 py-1.5 rounded-full bg-cream/10 border border-cream/15 hover:bg-cream/15"
              >
                {s}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        {[
          {
            i: <Brain className="h-5 w-5" />,
            t: "Models active",
            v: "14",
            d: "Lead scoring · NLP · Forecast · CV",
          },
          {
            i: <Database className="h-5 w-5" />,
            t: "Data sources",
            v: "32",
            d: "Portals · Ads · WhatsApp · ERP · Maps",
          },
          {
            i: <Shield className="h-5 w-5" />,
            t: "Security",
            v: "SOC 2",
            d: "RBAC · Audit logs · Encryption at rest",
          },
        ].map((s) => (
      ...
      <Card className="mt-4 p-5">
        ...
        <ul className="space-y-2">
          {[
            ...
            {
              t: "Lead response time slipped to 14m for Riverfront — assign +1 agent",
              c: "Operations",
            },
            ...
            <li
              key={i.t}
              className="flex items-center justify-between gap-3 p-3 rounded-lg hover:bg-muted/40"
            >
